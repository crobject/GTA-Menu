...
{
	if (m_toggle)
		Elem.Caption = Text + (std::string)(*m_toggle ? "[On]" : "[Off]");//, hudelem_color_t(0xFFFFFFFF), MENU_X_POSITION, MENU_Y_POSITION + ((float)Position * MENU_ITEM_DIFFERENCE), DEFAULT_OPTION_SCALE, 4);
	else
